...
const Ability = (props: AbilityProps) => {
  ...
  const [storedAbilities, setStoredAbilities] = useLocalStorage('abilities', {});
  const isStored = getProperty(storedAbilities, url) !== null;
  useEffect(() => {
    let isMounted = true;
    const fetchAbilityDetails = async () => {
      ...
      if (isMounted) {
        ...
        const lFlavor = localizeApiresponse(
        ...
        setStoredAbilities({...storedAbilities,[url]:abilityDetails})
      }
    };

    if (isStored) {
      const abilityDetails = getProperty(storedAbilities, url);
      const lName = localizeApiresponse(abilityDetails.names, locale, 'name');
      const lFlavor = localizeApiresponse(
        abilityDetails.flavor_text_entries,
        locale,
